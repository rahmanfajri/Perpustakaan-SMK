...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	*/
    ...
    $html_str .= '.mirror{display:block;-moz-transform: matrix(-1, 0, 0, 1, 0, 0);-webkit-transform: matrix(-1, 0, 0, 1, 0, 0);-o-transform:matrix(-1, 0, 0, 1, 0, 0);       }'."\n";
    $html_str .= '</style>'."\n";
    $html_str .= '</head>'."\n";
    if (!$_GET[mirror])   
{ $html_str .= '<body>'."\n"; }
    else                             
 { $html_str .= '<body class="mirror">'."\n"; }
    $html_str .= '<a href="#" onclick="window.print()">Print Again</a><br /><br />'."\n";
    $html_str .= '<table style="margin: 3; padding: 100;" cellspacing="0" cellpadding="0" border="1">'."\n";
    ...
    foreach ($chunked_card_arrays as $membercard_rows) {
       ...
       $html_str .= '<tr>'."\n";
        foreach ($membercard_rows as $card) {
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_id_label']?'':'<!--').'<p class="bio"><label class="bio_label">'.__('NIS').'</label><span>: </span>'.$card['member_id'].'</p>'.( $sysconf['print']['membercard']['include_id_label']?'':'-->').'';
          $html_str .= ''.( $sysconf['print']['membercard']['include_name_label']?'':'<!--').'<p class="bio"><label class="bio_label">'.__('Nama').'</label><span>: </span>'.$card['member_name'].'</p>'.( $sysconf['print']['membercard']['include_name_label']?'':'-->').'';
          $html_str .= ''.( $sysconf['print']['membercard']['include_pin_label']?'':'<!--').'<p class="bio"><label class="bio_label">'.__('NISN').'</label><span>: </span>'.$card['pin'].'</p>'.( $sysconf['print']['membercard']['include_pin_label']?'':'-->').'';
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<p class="bio_address"><label class="bio_label">'.__('Alamat').' / '.__('No. Hp').'</label><span style="float:left">: </span>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';
          ...
          $html_str .= '<p class="stamp officials">'.$sysconf['print']['membercard']['officials'].'<br />'.$sysconf['print']['membercard']['officials_id'].'</p></div></div><br></td>';
...
<div class="menuBoxInner printIcon">
	...
	<div class="sub_section">
        ...
        <a target="blindSubmit" href="<?php echo MWB; ?>membership/member_card_generator.php?action=print&mirror=1" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Mirror Member Cards for Selected Data'); ?></a>
