...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	*/
	...
	$member_q = $dbs->query("SELECT DATE_FORMAT(m.birth_date,'%d %M %Y')as tglahir, m.member_name, m.member_id, m.member_image, m.member_address, m.member_mail_address, m.member_email, m.inst_name, m.postal_code, m.pin, m.member_phone, m.expire_date, m.register_date, mt.member_type_name FROM member AS m
        LEFT JOIN mst_member_type AS mt ON m.member_type_id=mt.member_type_id
        WHERE m.member_id IN(".$member_ids.')');
    $member_datas = array();
    ...
    $html_str .= '.stamp_div{z-index:11;position: absolute;left: 200px;top:140px;margin-bottom: 34px;width: 118px;}'."\n";
    $html_str .= '.stamp{z-index:12;text-align: left;margin: 0px;}'."\n";
    $html_str .= '.city{z-index:13;font-size:7px;margin: 0px;}'."\n";
    $html_str .= '.title{z-index:14;font-size:8px;margin: 0px;}'."\n";
    $html_str .= '.officials{z-index:15;top: 0px;font-size: 8px;margin: 0px;}'."\n";
    $html_str .= '.sign_file_div{z-index:16;position: absolute;left: -15px;top: 10px;width:107px;height: 25px;}'."\n";
    $html_str .= '.stamp_file_div{z-index:17;position: absolute;left:-30px;top: 5px;width: 40px;height: 70px;}'."\n";
    $html_str .= '.exp_div{z-index:18;position: absolute;left: 120px;top: 142px;width:110px;height: 12px;font-size: 8px;text-align: left;}'."\n";
    $html_str .= '.barcode_div{z-index:19;position: absolute;left: 120px;top: 154px;width:150px;height: 42px;}'."\n";
    $html_str .= '</style>'."\n";
    $html_str .= '</head>'."\n";
    $html_str .= '<body>'."\n";
    ...
    foreach ($chunked_card_arrays as $membercard_rows) {
        ...
        foreach ($membercard_rows as $card) {
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_id_label']?'':'<!--').'<p class="bio"><label class="bio_label">'.__('NISN').'</label><span>: </span>'.$card['member_id'].'</p>'.( $sysconf['print']['membercard']['include_id_label']?'':'-->').'';
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<p class="bio_address"><label class="bio_label">'.__('Tempat').' , '.__('Tgl Lahir').'</label><span style="float:left">: </span>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';

          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<span class="label_address">'.$card['member_mail_address'].','.$card['tglahir'].'</span></p>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';
          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<p class="bio_address"><label class="bio_label">'.__('Address').' , '.__('Telp.').'</label><span style="float:left">: </span>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_expired_label']?'':'<!--').'<div class="exp_div">'.__('Expiry Date').' : '.long_date($card['expire_date']).'</div>'.( $sysconf['print']['membercard']['include_expired_label']?'':'-->').'';
          ...
          $html_str .= '<div class="stamp_file_div"><img class="" height="45px" width="45px" src="'.SWB.'/files/membercard/'.$sysconf['print']['membercard']['stamp_file'].'"></img></div>';
          $html_str .= '<div class="sign_file_div"><img class="" height="40px" width="100px" src="'.SWB.'/files/membercard/'.$sysconf['print']['membercard']['signature_file'].'"></img></div>';
          $html_str .= '<p class="stamp city">'.$sysconf['print']['membercard']['city'].', '.long_date($card['register_date']).'</p><p class="stamp title">'.$sysconf['print']['membercard']['title'].'</p><br>';
...
<div class="menuBoxInner printIcon">
	...
	<div class="sub_section">
		<div class="btn-group">
		<a target="blindSubmit" href="<?php echo MWB; ?>membership/kartu_tandatangan_belakang.php?action=clear" class="notAJAX btn btn-default" style="color: #f00;"><i class="glyphicon glyphicon-trash"></i>&nbsp;<?php echo __('Clear Print Queue'); ?></a>
		<a target="blindSubmit" href="<?php echo MWB; ?>membership/kartu_tandatangan_belakang.php?action=print" class="notAJAX btn btn-default"><i class="glyphicon glyphicon-print"></i>&nbsp;<?php echo __('Print Member Cards for Selected Data'); ?></a>
	    ...
	    <form name="search" action="<?php echo MWB; ?>membership/kartu_tandatangan_belakang.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?>:
...
$datagrid_result = $datagrid->createDataGrid($dbs, $table_spec, 10, $can_read);
if (isset($_GET['keywords']) AND $_GET['keywords']) {
